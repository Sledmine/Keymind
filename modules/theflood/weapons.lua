...
local animatedCrosshairsTagsCommon = {
    core.findTag("smg_m7a", tagClasses.weaponHudInterface)
}

local animatedCrosshairsTagsLarger = {
    core.findTag("assault_rifle_ma38", tagClasses.weaponHudInterface)
}

-- Crosshair SMG
function weapons.animateCrosshairs()
    for crosshairNumber, crosshairTag in pairs(animatedCrosshairsTagsCommon) do
        if (crosshairTag) then
            local weaponHud = blam.weaponHudInterface(crosshairTag.path)
...
function weapons.animateCrosshairsAr()
    for crosshairNumber, crosshairTag in pairs(animatedCrosshairsTagsLarger) do
        if (crosshairTag) then
            local weaponHud = blam.weaponHudInterface(crosshairTag.path)
            local crosshairs = weaponHud.crosshairs
            local player = blam.biped(get_dynamic_player())
